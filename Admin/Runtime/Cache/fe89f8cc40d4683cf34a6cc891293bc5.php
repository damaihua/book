...
$("#sys").addClass("current");$("#sys_admin_user").addClass("current");</script>				
	...
	<div class="content-box"><!-- Start Content Box -->
				...
				<div class="content-box-header">
					...
					<h3 style="cursor: s-resize;">积分规则修改</h3>
...
<tbody><tr class="alt-row">
	...
	<form name="form1" action="__APP__/score/do_score_role_edit" onsubmit="return checkSubmit();" method="post">
	
  <table cellpadding="1" cellspacing="1" border="0" width="98%">
          <tbody><tr> 
          
            <td height="30" width="16%">事件：</td>
            <td style="text-align:left;" width="84%"><input name="id" type="hidden" id="id"  value="<?php echo ($data["id"]); ?>">
            <input name="name" type="text" id="name" style="width:200px" value="<?php echo ($data["name"]); ?>" size="16"></td>
          </tr>
          <tr> 
          
            <td height="30" width="16%">处理：</td>
            <td style="text-align:left;" width="84%">
            <select name='type'>
            <option value='1' 
            <?php if($data["type"] == 1 ): ?>selected
<?php else: endif; ?>

    
            >奖励</option>
             <option value='0'  <?php if($data["type"] == 0 ): ?>selected
<?php else: endif; ?>>惩罚</option>
            </select></td>
          </tr>
          <tr> 
            <td height="30">奖罚积分：</td>
            <td style="text-align:left;"><input name="score" type="text" id="score" style="width:200px" value="<?php echo ($data["score"]); ?>" size="16"></td>
          </tr>
