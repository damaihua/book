 ...
 <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?><tr align="center" height="26"> 
  ...
  <td>
   <?php if($user["user_type"] == 1 ): ?>普通会员
    <?php else: ?><font color="#FF0000" >签约作者</font><?php endif; ?>
  </td>
     <td width="6%">
     <?php if($user["user_status"] == 1 ): ?>正常
    <?php else: ?><font color="#FF0000" >禁止</font><?php endif; ?>

     </td>
  
    <td width="6%">1000</td>
  <td>
  <?php echo (date('Y-m-d',$user["reg_time"])); ?>   	</td>
  <td align="center">
    <a href="http://localhost/dede/dede/member_view.php?id=1">修改</a> | 
    <a href="__APP__/member/member_del?user_id=<?php echo ($user["user_id"]); ?>" onclick="return confirm('确定要删除?')">删除</a></td>
