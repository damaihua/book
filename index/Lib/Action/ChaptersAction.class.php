...
class ChaptersAction extends Action {
		...
		if($chapter[0]['charge']){
			...
			if(isset($_SESSION['user_name']) && !empty($_SESSION['user_name'])){
				...
				if(in_array($chapter_id,$orders)){
					...
					//显示成功后，小说的点击加一
					$book=M('Book');
					$book_id=$book_info[0]['book_id'];
					$hits=$book->field('hits')->find($book_id);
					$hit=$hits['hits'];
					$data['hits']=$hit+1;
					$where='book_id='.$book_id;
					...
					if(($money-$much)>0){
						...
						$data['chapter_id']=$_GET['chapter_id'];
						$res=$order->add($data);
						//购买成功后，显示阅读页面
						if($res){
							$this->assign('chapter',$chapter);
	...
	public function comment(){
		...
		if(isset($_SESSION['user_name']) && !empty($_SESSION['user_name'])){
			...
			$data['content']=trim($_POST['pcontent']);
