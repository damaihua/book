...
<head>
<title>收信箱--<?php echo (SITE_NAME); ?></title>
...
<caption>收件箱</caption>
  <tr>
    <th width="5%">
	<input type="checkbox" id="checkall" name="checkall" value="checkall" onclick="javascript: for(var i=0;i<this.form.elements.length;i++){ if(this.form.elements[i].name != 'checkkall') this.form.elements[i].checked = this.form.checkall.checked; }"></th>
    <th width="20%">发件人</th>
    <th width="60%">标题</th>
    <th width="15%">日期</th>
  </tr>
...
<?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
    ...
    <td class="even">
		<span class="hottext"><?php echo ($vo["user_name"]); ?></span>
	</td>
    <td class="odd">
		<a href="__URL__/detail/letter_id/<?php echo ($vo["letter_id"]); ?>"					style="color:<?php echo ($vo[status]?$colorA:$colorB); ?>">
			<?php echo ($vo["letter_title"]); ?>
			<span><?php echo ($vo['status']?$staB:$staA); ?></span>
		</a>
	</td>
